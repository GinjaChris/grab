#I basically took this script: https://github.com/Beek-Kefyalew/Python-Banner-Grabber/blob/master/bannerGra>
        ...
        for port in PortList:
                ...
                if port == 80 or port == 443:
                        try:
                                ...
                                response=requests.head(str(PortNames[port])+"://"+dest, headers=headers, ti>
                                print(response.status_code, response.reason)
                                print(response.headers)
                        except:
                                print("[ - ] Something went wrong....probably the server didn't respond cor>
                                continue
                else:
                        if  banner:
